...
interface NewsletterDraftFormProps {
  title: string;
  onResult: (l: string, c: string) => void;
  onBack: () => void;
}

export default function NewsletterDraftForm({ title, onResult, onBack }: NewsletterDraftFormProps) {
  const [loading, setLoading] = useState(false);
  const [draft, setDraft] = useState<{ subject: string; content: string; meta_description: string } | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success('뉴스레터 초안이 생성되었습니다');
      } else {
  ...
  const handleFinalize = () => {
    if (!draft) return;
    const result = `**Subject:** ${draft.subject}\n\n${draft.content}`;
    onResult('📰 뉴스레터 최종화', result);
    toast.success('뉴스레터가 최종화되었습니다');
  };
  ...
  const handleFigmaPush = async () => {
    ...
    try {
      const res = await fetch('/api/figma/push', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ bannerId, html: headerHtml, size: '600x200' }),
      });
      if (res.ok) {
  ...
  return (
    <div className="space-y-3">
      ...
      {!draft && (
        ...
        >
          {loading ? <><Loader2 className="size-4 animate-spin" /> 초안 생성 중...</> : '뉴스레터 초안 생성'}
        </button>
      )}
      ...
      {draft && (
        <>
          ...
          {headerHtml && (
            <div className="mt-2">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-medium text-muted-foreground">헤더이미지 (600x200)</span>
                {bannerId && (
                  <button
          ...
          <div className="rounded-lg border border-border bg-white dark:bg-[#252326] p-5 max-h-80 overflow-auto">
            <h2 className="text-base font-semibold text-foreground mb-1">{draft.subject}</h2>
            <p className="text-xs text-muted-foreground mb-3">{draft.meta_description}</p>
