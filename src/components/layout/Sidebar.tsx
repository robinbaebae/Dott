...
export default function Sidebar() {
  ...
  if (pathname === '/' && status !== 'authenticated' && !isDemo && !isGuest) {
    return null;
  }
  ...
  return (
    ...
    >
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 ${collapsed ? 'justify-center px-1' : 'px-3'} py-1.5 rounded-xl text-[13px] transition-colors ${
                  active
      ...
      <div className="px-2.5 pb-3 space-y-0.5">
        <div className={`flex items-center gap-2 ${collapsed ? 'justify-center px-1' : 'px-3'} py-1.5`}>
          <ThemeToggle />
        </div>
        {isDemo ? (
          <>
